...
for i = 1:length(CleanMuscles)
    ...
    if ~isnan(MusMapCond(i))
        ...
        if contains(NameStr,'All')
            switch iCase
                case 7 %all conditions/all speeds
                    iDs = 1:Ncycles;
                case 8 % INTACT, all speeds
                    iDs = 1:3;
                case 9 % SPINAL, all speeds
                     iDs = 4:6;
                case 10 %all conditions, 0.4
                     iDs = [1,4];
                case 11 %all conditions, 0.7
                     iDs = [2,5];
                case 12 %all conditions, 1.0
                     iDs = [3,6];
            end

            for iC = 1:Ncycles
                Sw2Plot = Sw2Plots(iDs(iC)) + NTime1Cycle*(iC-1);
...
for i = 1:N_syn
    ...
    if doPlotEach ~= 0
        plot(squeeze(H_all3D_s(:,i,1:doPlotEach:end)))
    end
        if contains(NameStr,'All')
            switch iCase
                case 7 %all conditions/all speeds
