function [hComb] = plotCombineFigure3(FigName,N_syn,H_4D,W_4D,...
...
switch NConditions
    case 2
        %ColorD = {'b','g',lightBrown};
        ColorNames = {'Blue','Green','Brown'};
        %ColorInd = {[0,0,0.5],[0,0.5,0],darkBrown}; %dark
                
        ColorD = [{'b','g'},repmat({lightBrown},1,12-NConditions)];
        ColorInd = [{[0,0,0.5],[0,0.5,0]},repmat({darkBrown},1,12-NConditions)];

    case 3
     %   ColorD = {'b','g','r',lightBrown};
        ColorNames = {'Blue','Green','Red','Brown'};
     %   ColorInd = {[0,0,0.5],[0,0.5,0],[0.5,0,0],darkBrown}; %dark
        ColorD = [{'b','g','r'},repmat({lightBrown},1,12-NConditions)];
        ColorInd = [{[0,0,0.5],[0,0.5,0],[0.5,0,0]},repmat({darkBrown},1,12-NConditions)];
    case 6
         ColorD = {'b','g','r',lightBlue,lightGreen,lightRed,lightBrown};
        ColorNames = {'Blue','Green','Red','lightBlue','lightGreen','lightRed','Brown'};
        ColorInd = {[0,0,0.5],[0,0.5,0],[0.5,0,0],[0,0,0.5],[0,0.5,0],[0.5,0,0],darkBrown}; %dark
end
...
for i = 1:N_syn
    ...
    %HSyn = squeeze(H_4D(:,i,:,:));
    HSyn = squeeze(H_4D(:,i,:,[UsedConditions,CombCondition]));
    ...
    for iCond = 1:NConditions
        Sw2Plot = Sw2Plots(UsedConditions(iCond)) + NTime*(iCond-1);
        ...
        line([NTime*(iCond-1) NTime*(iCond-1)],[0 1],'Color','k','LineStyle','-','LineWidth',1)
    end

    ConditionsID = [UsedConditions,CombCondition];
    ...
    if iCond >NConditions
        ...
        if Nskip~=NShuffles
            plot(CtCond{itCondID},squeeze(Hsh(CindCond{iCondID},:,iCond)),'color',ColorD{iCond},'LineWidth',0.01);
        end

        plot(CtCond{itCondID},Hm(CindCond{iCondID},iCond),'Color',ColorInd{iCond},'LineWidth',0.6);

        plot1 = fill(Cx2{itCondID}, inBetweenPlus(CindCondInBetween{iCondID},iCond),ColorD{iCond});
        plot1.FaceAlpha = .2;
        plot1.EdgeColor ='none';

        plot2 = fill(Cx2{itCondID}, inBetweenMinus(CindCondInBetween{iCondID},iCond),ColorD{iCond});
        plot2.FaceAlpha = .2;
    ...
    WSyn = squeeze(W_4D(i,MusMap,:,[UsedConditions,CombCondition]));
    ...
    b= bar(Wm);
    for iCond = 1:NConditions + 1 % [UsedConditions,CombCondition] %
